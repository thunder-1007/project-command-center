...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  async function signOut() { sessionStorage.removeItem("formiva_demo"); sessionStorage.removeItem("formiva_demo_email"); await supabase.auth.signOut(); navigate({ to: "/auth", replace: true }); }
  return <div className="min-h-screen bg-background text-foreground">
    <aside className="fixed inset-y-0 left-0 hidden w-[250px] flex-col border-r border-border bg-sidebar lg:flex">
      <div className="border-b border-border px-5 py-5"><div className="flex items-center gap-3"><img src="/formiva-mark.png" className="h-9 w-9 object-contain" /><div><div className="font-semibold tracking-[0.18em] text-primary">FORMIVA</div><div className="text-[11px] text-muted-foreground">CaseFlow / mission control</div></div></div></div>
